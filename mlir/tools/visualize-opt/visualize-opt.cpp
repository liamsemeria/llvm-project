...
int main(int argc, char **argv) {
    ...
    // if a custom pipeline is found, add alternating calls to the visualization pass
    for (int i = 1; i < argc; i++) {
        std::string s = argv[i];
        std::string::size_type n = s.find("builtin.module(");
        // look for an argument mentioning a pass pipeline
        if (n != -1) {
          std::string r = s.substr(0,n) + "builtin.module(test-dialect-counts,";
          n = s.find('(');
          ...
          // otherwise just copy the old argument
          new_argv[i] = new char[strlen(argv[i])+1];
          strcpy(new_argv[i],argv[i]);
        }

    }
    llvm::outs() << "\033[1;38;5;147mRunning Visualizer...\033[0m\n";

    return mlir::asMainReturnCode(mlir::MlirOptMain(argc, new_argv, "MLIR modular optimizer driver with visuals\n", registry));
}